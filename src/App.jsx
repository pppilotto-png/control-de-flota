            ...
            <Card style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px 0" }}>
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: C.muted, marginTop: -8, marginBottom: 10 }}>Completá una línea por pedido.</div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
            ...
            <Card style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px 0" }}>
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: C.muted, marginTop: -8, marginBottom: 10 }}>Completá una línea por costo.</div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
...
function RelatoriosPage({ veiculos, viagens, custos, abastecimentos, manutencoes, pedidos, tarifas, sucursales }) {
  ...
  const REPORTES_MENU = [
    ...
    { id: "tipoFlete", icon: LayoutGrid, title: "Tipo de flete", desc: "Pedidos e ingresos agrupados por tipo de flete, por mes." },
  ...
  const porTipoFlete = useMemo(() => {
    const map = {};
    TIPOS_FRETE.forEach((t) => { map[t] = { cantidad: 0, facturas: 0, ingreso: 0 }; });
    pedidosTipoFleteF.forEach((p) => {
      const t = p.tipoFlete || "Sin tipo";
      if (!map[t]) map[t] = { cantidad: 0, facturas: 0, ingreso: 0 };
      map[t].cantidad += 1;
      map[t].facturas += Number(p.valorFatura || 0);
      map[t].ingreso += freightRevenue(p, tarifas);
    });
    return Object.entries(map).filter(([, v]) => v.cantidad > 0);
  }, [pedidosTipoFleteF, tarifas]);
  const totalPedidosTipoFlete = porTipoFlete.reduce((s, [, v]) => s + v.cantidad, 0);
  const totalIngresoTipoFlete = porTipoFlete.reduce((s, [, v]) => s + v.ingreso, 0);
    ...
    <div>
      ...
      <div id="report-print-area">
        ...
        {vista === "tipoFlete" && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 14, marginBottom: 20 }}>
              <KPI icon={Package} label={mesFiltro ? `Pedidos en ${mesFiltro}` : "Pedidos en el período"} value={totalPedidosTipoFlete} />
              <KPI icon={Receipt} label="Ingreso por fletes" value={fmtMoney(totalIngresoTipoFlete)} />
            </div>
            <Card style={{ marginBottom: 16 }}>
              <ChartTitle>Ingreso por tipo de flete</ChartTitle>
              {porTipoFlete.length === 0 ? <EmptyState icon={LayoutGrid} text="No hay pedidos para el mes seleccionado." /> : (
                <div style={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={porTipoFlete.map(([tipo, v]) => ({ name: tipo, value: v.ingreso }))}>
                      <CartesianGrid stroke={C.border} vertical={false} />
                      <XAxis dataKey="name" tick={{ fill: C.muted, fontSize: 11 }} />
                      <YAxis tick={{ fill: C.muted, fontSize: 11 }} />
                      <Tooltip formatter={(v) => fmtMoney(v)} contentStyle={tooltipStyle} />
            ...
            <Card style={{ padding: 0 }}>
              <div style={{ padding: "14px 16px 0" }}><ChartTitle>Detalle por tipo de flete</ChartTitle></div>
              {porTipoFlete.length === 0 ? <div style={{ padding: 16 }}><EmptyState icon={LayoutGrid} text="No hay pedidos para el mes seleccionado." /></div> : (
                <Table
                  headers={["Tipo de flete", "Cantidad de pedidos", "Valor facturas", "Ingreso flete"]}
                  rows={porTipoFlete.map(([tipo, v]) => [
                    tipo, v.cantidad, fmtMoney(v.facturas),
                    <span style={{ color: C.green, fontWeight: 700 }}>{fmtMoney(v.ingreso)}</span>,
                  ])}
                />
              )}
            </Card>
            ...
            <div style={{ marginTop: 16 }}>
              <ChartTitle>Todos los pedidos {mesFiltro ? `de ${mesFiltro}` : "del período"}</ChartTitle>
              {pedidosTipoFleteF.length === 0 ? (
                <EmptyState icon={Receipt} text="No hay pedidos para el mes seleccionado." />
              ) : (
                <Card style={{ padding: 0 }}>
                  <Table
                    headers={["Viaje N°", "Fecha", "Vehículo", "Tipo de flete", "Factura", "Pedido", "Cliente", "Valor factura", "Ingreso flete"]}
                    rows={[...pedidosTipoFleteF]
                      .sort((a, b) => (a.tipoFlete || "").localeCompare(b.tipoFlete || ""))
                      .map((p) => {
                        const v = viagens.find((x) => x.id === p.viagemId);
                        return [
                          v?.numero ?? "—",
