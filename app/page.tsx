...
type Trip = { id: number; branch: string; startDate: string; endDate?: string; driver: string; helper?: string; vehicle: string; status: TripStatus; kmInitial: number; kmFinal: number; distanceSource?: DistanceSource; estimatedKm?: number; kmValidated?: boolean; validatedBy?: string; gpsConsumedLiters?: number; fuelInitialLevel?: number; fuelFinalLevel?: number; fuelDataValidated?: boolean; orders: Order[] } & ImportMetadata;
...
export default function Home() {
  ...
  function finishTrip(event: FormEvent<HTMLFormElement>) {
    ...
    setTrips(trips.map((trip) => trip.id === finishingTrip.id ? { ...trip, endDate, kmFinal: distanceSource === "Odómetro" ? kmFinal : trip.kmFinal, distanceSource, estimatedKm: distanceSource === "Odómetro" ? undefined : estimatedKm, kmValidated: distanceSource === "Odómetro" ? undefined : form.get("kmValidated") === "on", validatedBy: distanceSource === "Odómetro" ? undefined : String(form.get("validatedBy") || "") || undefined, gpsConsumedLiters: Number(form.get("gpsConsumedLiters") || 0) || trip.gpsConsumedLiters, fuelInitialLevel: form.get("fuelInitialLevel") !== null && String(form.get("fuelInitialLevel")) !== "" ? Number(form.get("fuelInitialLevel")) : trip.fuelInitialLevel, fuelFinalLevel: form.get("fuelFinalLevel") !== null && String(form.get("fuelFinalLevel")) !== "" ? Number(form.get("fuelFinalLevel")) : trip.fuelFinalLevel, fuelDataValidated: form.get("fuelDataValidated") === "on", status: "Finalizado" } : trip));
...
function BonusesHistoryModule({ trips, vehicles, fuelEntries, cycles, reviews, setReviews, helperAssignments, helperReviews, setHelperReviews, closures, setClosures, readOnly }: {
  ...
  return <div className="bonus-module bonus-history-module">
    ...
    <article className="bonus-consolidated-report bonus-history-print standalone-print-report"><header className="bcr-header"><h1>INFORME MENSUAL DE BONIFICACIONES</h1><p>Periodo: {monthLabel(month)} · Estado: {closed ? "Cerrado" : "En evaluación"}</p></header><section className="bcr-section bcr-compact"><table><thead><tr><th>Nombre</th><th>Categoría</th><th>Criterios</th><th>Resultados</th><th>Total</th></tr></thead><tbody>{shownEntries.map((entry) => { const criteria = ["Descarga", "Sin devolución", ...(entry.personType === "Chofer" ? ["Consumo promedio"] : [])]; const results = [entry.unloadingMet, entry.damageMet, ...(entry.personType === "Chofer" ? [entry.fuelScore > 0] : [])]; return <tr key={`${entry.personType}-${entry.name}`}><td><strong>{entry.name}</strong></td><td>{entry.personType} {entry.category}</td><td>{criteria.join(" · ")}</td><td>{results.map((met, index) => <Fragment key={criteria[index]}><span className={met ? "fulfilled" : "not-fulfilled"}>{met ? "Cumplido" : "No cumplido"}</span>{index < results.length - 1 ? " · " : ""}</Fragment>)}</td><td className="bcr-total">{money.format(entry.total)}</td></tr>; })}</tbody></table></section><section className="bcr-summary"><h2>RESUMEN GENERAL</h2><table><tbody><tr><td>Total choferes</td><td>{money.format(driverTotal)}</td></tr><tr><td>Total ayudantes</td><td>{money.format(helperTotal)}</td></tr><tr className="grand-total"><td>TOTAL GENERAL</td><td>{money.format(driverTotal + helperTotal)}</td></tr></tbody></table></section><footer>FreteControl ERP · Valores expresados en guaraníes (PYG)</footer></article>
  </div>;
}
