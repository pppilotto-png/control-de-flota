...
type FuelCycle = { id: string; vehicle: string; startOdometer: number; endOdometer: number; startDate?: string; endDate?: string; distanceMethod?: FuelDistanceMethod; distance: number; cost: number; liters: number; availableLiters: number; consumedLiters: number; consumptionMethod: FuelCalculationMethod; consumptionValidated: boolean; entryIds: number[]; allocations: { tripId: number; km: number; consumedLiters: number; value: number }[] };
...
const defaultFuelTargets: Record<string, number> = { AAUC019: 7.38, AASN159: 6.42, ABAD083: 6.11, ABAD112: 9, ABBB263: 4.71 };
  ...
  Array.from(new Set(entries.map((entry) => entry.vehicle))).forEach((vehicle) => {
    ...
    for (let index = 0; index < fullIndexes.length - 1; index += 1) {
      ...
      const allocations = trips.filter((trip) => trip.vehicle === vehicle && trip.kmFinal > trip.kmInitial).map((trip) => {
        ...
        return { tripId: trip.id, km, consumedLiters: liters * km / distance, value: Math.round(cost * km / distance) };
    ...
    for (let index = 0; index < alternativeFullIndexes.length - 1; index += 1) {
      ...
      const cycleTrips = trips.filter((trip) => trip.vehicle === vehicle && tripDistance(trip) > 0 && (!isEstimatedTrip(trip) || trip.kmValidated) && trip.startDate >= start.date && (trip.endDate ?? trip.startDate) <= end.date).sort((a, b) => a.startDate.localeCompare(b.startDate) || a.id - b.id);
      ...
      const allocations = distance > 0 ? cycleTrips.map((trip) => {
        ...
        const tripConsumedLiters = calculationMethod === "GPS / consumo informado" ? Number(trip.gpsConsumedLiters || 0) : consumedLiters * km / distance;
        const consumedPercentage = availableLiters > 0 ? tripConsumedLiters / availableLiters : 0;
        return { tripId: trip.id, km, consumedLiters: tripConsumedLiters, value: Math.round(cost * consumedPercentage) };
      }).filter((allocation) => allocation.km > 0) : [];
...
export default function Home() {
  ...
  const setRequests = setServiceRequests;
  ...
  function saveFuel(event: FormEvent<HTMLFormElement>) {
    ...
    const saved: FuelEntry = {
      ...
      gpsConsumedLiters: editingFuel?.gpsConsumedLiters,
      cycleInitialFuelLevel: editingFuel?.cycleInitialFuelLevel,
      cycleFinalFuelLevel: editingFuel?.cycleFinalFuelLevel,
      consumptionValidated: calculationMethod === "Hodómetro / tanque lleno" ? true : editingFuel?.consumptionValidated,
  ...
  function saveTrip(event: FormEvent<HTMLFormElement>) {
    ...
    const distanceSource = String(form.get("distanceSource") || "Odómetro") as DistanceSource;
    const saved: Trip = { id: editingTrip?.id ?? (trips.length ? Math.max(...trips.map((trip) => trip.id)) + 1 : 1), branch: String(form.get("branch") || ""), startDate: String(form.get("startDate") || ""), endDate: String(form.get("endDate") || "") || undefined, driver: String(form.get("driver") || "Por definir"), helper: String(form.get("helper") || "") || undefined, vehicle: String(form.get("vehicle") || "Por definir"), status: String(form.get("status") || "Pendiente") as TripStatus, kmInitial: Number(form.get("kmInitial") || 0), kmFinal: Number(form.get("kmFinal") || 0), distanceSource, estimatedKm: distanceSource === "Odómetro" ? undefined : Number(form.get("estimatedKm") || 0), kmValidated: distanceSource === "Odómetro" ? undefined : form.get("kmValidated") === "on", validatedBy: distanceSource === "Odómetro" ? undefined : String(form.get("validatedBy") || "") || undefined, gpsConsumedLiters: Number(form.get("gpsConsumedLiters") || 0) || undefined, idleConsumedLiters: Number(form.get("idleConsumedLiters") || 0) || undefined, fuelInitialLevel: form.get("fuelInitialLevel") !== null && String(form.get("fuelInitialLevel")) !== "" ? Number(form.get("fuelInitialLevel")) : undefined, fuelFinalLevel: form.get("fuelFinalLevel") !== null && String(form.get("fuelFinalLevel")) !== "" ? Number(form.get("fuelFinalLevel")) : undefined, fuelDataValidated: form.get("fuelDataValidated") === "on", orders, importBatchId: editingTrip?.importBatchId, importedAt: editingTrip?.importedAt, importFileName: editingTrip?.importFileName };
    ...
    if (saved.status === "Finalizado" && (!saved.endDate || (distanceSource === "Odómetro" ? saved.kmFinal <= saved.kmInitial : !saved.estimatedKm))) {
      setTripTab("data");
      setTripFormError("Para finalizar, informe la fecha final y la distancia recorrida.");
      return;
    }
  ...
  function finishTrip(event: FormEvent<HTMLFormElement>) {
    ...
    const estimatedKm = Number(form.get("estimatedKm") || 0);
    const gpsConsumedLiters = Number(form.get("gpsConsumedLiters") || 0) || finishingTrip.gpsConsumedLiters;
    const idleConsumedLiters = Number(form.get("idleConsumedLiters") || 0) || finishingTrip.idleConsumedLiters;
    if (endDate < finishingTrip.startDate || (distanceSource === "Odómetro" ? kmFinal <= finishingTrip.kmInitial : estimatedKm <= 0)) {
  ...
  const selectedVehicleProfile = vehicles.find((vehicle) => vehicle.plate === vehicleDraft);
  const selectedOdometerBroken = selectedVehicleProfile?.odometerStatus === "Averiado";
  const selectedFuelCalculationMethod = calculationMethodFor(selectedVehicleProfile);
  ...
  const finishingFuelCalculationMethod = calculationMethodFor(finishingVehicleProfile);
...
function printStandaloneReport(selector: string, title: string, fallbackMode: string, orientation: "portrait" | "landscape" = "landscape") {
...
function BonusesHistoryModule({ trips, vehicles, fuelEntries, cycles, reviews, setReviews, helperAssignments, helperReviews, setHelperReviews, closures, setClosures, readOnly }: {
  trips: Trip[]; vehicles: Vehicle[]; fuelEntries: FuelEntry[]; cycles: FuelCycle[];
  ...
  return <div className="bonus-module bonus-history-module">
    ...
    {reportDriver && <div className="modal-backdrop bonus-driver-preview" onMouseDown={() => setReportDriverName("")}><div className="trip-report" onMouseDown={(event) => event.stopPropagation()}><div className="report-toolbar"><button className="secondary" onClick={() => setReportDriverName("")}>Cerrar</button><button className="primary" onClick={() => printStandaloneReport(".bonus-driver-report", `Bonificación — ${reportDriver.name} — ${monthLabel(month)}`, "printing-bonus-report", "portrait")}><Icon name="report"/>Imprimir / PDF</button></div><article className="individual-bonus-report bonus-driver-report standalone-print-report">
      ...
      <section className="ibr-section"><h2><b>2</b> BONIFICACIÓN DEL CHOFER</h2><h3>{reportDriver.name} <span>— Chofer {reportDriver.category.toLowerCase()}</span></h3><table className="ibr-bonus-table"><thead><tr><th>Criterio</th><th>Resultado</th><th>Bonificación</th></tr></thead><tbody><tr><td>Descarga de mercaderías</td><td><em>{reportDriver.unloadingMet ? "Cumplido" : "No cumplido"}</em></td><td>{money.format(reportDriver.unloadingBonus)}</td></tr><tr><td>Sin devolución por averías</td><td><em>{reportDriver.damageMet ? "Cumplido" : "No cumplido"}</em></td><td>{money.format(reportDriver.damageBonus)}</td></tr><tr><td>Meta de consumo promedio</td><td><em>{reportDriver.fuelScore === 1 ? "Cumplido" : reportDriver.fuelScore ? "Parcial" : "No cumplido"} — {reportDriver.consumption ? `${reportDriver.consumption.toFixed(2)} km/L` : "Sin datos"}</em></td><td>{money.format(reportDriver.fuelBonus)}</td></tr><tr className="total"><td>Total del chofer</td><td></td><td>{money.format(reportDriver.total)}</td></tr></tbody></table></section>
      <section className="ibr-section"><h2><b>3</b> BONIFICACIÓN DEL AYUDANTE</h2>{reportHelper ? <><h3>{reportHelper.name} <span>— Ayudante {reportHelper.category.toLowerCase()}</span></h3><table className="ibr-bonus-table"><thead><tr><th>Criterio</th><th>Resultado</th><th>Bonificación</th></tr></thead><tbody><tr><td>Descarga de mercaderías</td><td><em>{reportHelper.unloadingMet ? "Cumplido" : "No cumplido"}</em></td><td>{money.format(reportHelper.unloadingBonus)}</td></tr><tr><td>Sin devolución por averías</td><td><em>{reportHelper.damageMet ? "Cumplido" : "No cumplido"}</em></td><td>{money.format(reportHelper.damageBonus)}</td></tr><tr className="total"><td>Total del ayudante</td><td></td><td>{money.format(reportHelper.total)}</td></tr></tbody></table></> : <p className="ibr-empty">No hay ayudante vinculado a este chofer.</p>}</section>
      <section className="ibr-section"><h2><b>4</b> RESUMEN DEL PAGO</h2><table className="ibr-summary"><tbody><tr><td>Chofer</td><td>{money.format(reportDriver.total)}</td></tr><tr><td>Ayudante</td><td>{money.format(reportHelper?.total ?? 0)}</td></tr><tr className="total"><td>Total general del equipo</td><td>{money.format(reportDriver.total + (reportHelper?.total ?? 0))}</td></tr></tbody></table></section><footer>FreteControl ERP · Informe mensual de bonificaciones</footer>
    </article></div></div>}
    <article className="bonus-consolidated-report bonus-history-print standalone-print-report">
      <header className="bcr-header"><h1>INFORME MENSUAL DE BONIFICACIONES</h1><p>Periodo: {monthLabel(month)}</p></header>
      <section className="bcr-section"><h2>Choferes</h2><table><thead><tr><th>Nombre</th><th>Categoría</th><th>Criterio</th><th>Resultado</th><th>Bonificación</th><th>Total</th></tr></thead>
        {shownEntries.filter((entry) => entry.personType === "Chofer").map((entry) => <tbody className="bcr-person" key={`pdf-${entry.personType}-${entry.name}`}>
          <tr><td rowSpan={3} className="bcr-name">{entry.name}</td><td rowSpan={3}>{entry.category}</td><td>Descarga de mercaderías</td><td><span className={entry.unloadingMet ? "fulfilled" : "not-fulfilled"}>{entry.unloadingMet ? "Cumplido" : "No cumplido"}</span></td><td>{money.format(entry.unloadingBonus)}</td><td rowSpan={3} className="bcr-total">{money.format(entry.total)}</td></tr>
          <tr><td>Sin devolución por averías</td><td><span className={entry.damageMet ? "fulfilled" : "not-fulfilled"}>{entry.damageMet ? "Cumplido" : "Pendiente"}</span></td><td>{money.format(entry.damageBonus)}</td></tr>
          <tr><td>Promedio de consumo</td><td><span className={entry.fuelScore > 0 ? "fulfilled" : "not-fulfilled"}>{entry.fuelScore === 1 ? "Cumplido" : entry.fuelScore > 0 ? `Parcial (${Math.round(entry.fuelScore * 100)}%)` : "No cumplido"}</span></td><td>{money.format(entry.fuelBonus)}</td></tr>
        </tbody>)}
        {!shownEntries.some((entry) => entry.personType === "Chofer") && <tbody><tr><td colSpan={6}>No hay choferes evaluados en este periodo.</td></tr></tbody>}
      </table></section>
      <section className="bcr-section"><h2>Ayudantes</h2><table><thead><tr><th>Nombre</th><th>Categoría</th><th>Criterio</th><th>Resultado</th><th>Bonificación</th><th>Total</th></tr></thead>
        {shownEntries.filter((entry) => entry.personType === "Ayudante").map((entry) => <tbody className="bcr-person" key={`pdf-${entry.personType}-${entry.name}`}>
          <tr><td rowSpan={2} className="bcr-name">{entry.name}</td><td rowSpan={2}>{entry.category}</td><td>Descarga de mercaderías</td><td><span className={entry.unloadingMet ? "fulfilled" : "not-fulfilled"}>{entry.unloadingMet ? "Cumplido" : "No cumplido"}</span></td><td>{money.format(entry.unloadingBonus)}</td><td rowSpan={2} className="bcr-total">{money.format(entry.total)}</td></tr>
          <tr><td>Sin devolución por averías</td><td><span className={entry.damageMet ? "fulfilled" : "not-fulfilled"}>{entry.damageMet ? "Cumplido" : "Pendiente"}</span></td><td>{money.format(entry.damageBonus)}</td></tr>
        </tbody>)}
        {!shownEntries.some((entry) => entry.personType === "Ayudante") && <tbody><tr><td colSpan={6}>No hay ayudantes evaluados en este periodo.</td></tr></tbody>}
      </table></section>
      <section className="bcr-summary"><h2>RESUMEN GENERAL</h2><table><tbody><tr><td>Total choferes</td><td>{money.format(driverTotal)}</td></tr><tr><td>Total ayudantes</td><td>{money.format(helperTotal)}</td></tr><tr className="grand-total"><td>TOTAL GENERAL</td><td>{money.format(driverTotal + helperTotal)}</td></tr></tbody></table></section>
      <footer>FreteControl ERP · Valores expresados en guaraníes (PYG)</footer>
    </article>
  </div>;
}

function ResultsModule({ trips, vehicleFilter, rates, costs, fuelByTrip, fuelCycles, fuelEntries, vehicles, bonusClosures, onReport }: {
  trips: Trip[];
  ...
  const rows = reportTrips.map((trip) => {
    ...
    const fullKm = tripDistance(trip);
  ...
  const averageFuelConsumption = fuelConsumptionTotals.liters > 0
    ? fuelConsumptionTotals.distance / fuelConsumptionTotals.liters
    : 0;
  ...
  return <section className="results-layout">
    ...
    <div className={`result-kpis ${area === "executive" ? "executive-kpis" : ""}`}>
      ...
      {area === "executive" && <article className="fuel-average-card"><small>Promedio de combustible</small><strong>{averageFuelConsumption > 0 ? `${averageFuelConsumption.toFixed(2).replace(".", ",")} km/L` : "Sin datos"}</strong><span>{averageFuelConsumption > 0 ? "Consumo consolidado de ciclos validados" : "Aguardando ciclos cerrados y validados"}</span></article>}
    ...
    {area === "operations" && <section className="table-card executive-driver-table operational-report"><div className="card-heading"><div><p className="eyebrow">Rendimiento operativo</p><h2>Detalle por viaje</h2><p className="muted">Fechas, chofer, vehículo, consumo y productividad de cada operación.</p></div><strong>{operationalRows.length} viajes</strong></div><div className="table-scroll"><table><thead><tr><th>Fecha inicio</th><th>Fecha fin</th><th>Nombre del chofer</th><th>Chapa</th><th>Combustible (km/L)</th><th>Capacidad / tipo</th><th>Cantidad de entregas</th><th>Kilómetros recorridos</th><th>Mercadería transportada</th></tr></thead><tbody>{operationalRows.map((item) => <tr key={item.trip.id}><td>{new Date(`${item.trip.startDate}T12:00:00`).toLocaleDateString("es-PY")}</td><td>{item.trip.endDate ? new Date(`${item.trip.endDate}T12:00:00`).toLocaleDateString("es-PY") : "En curso"}</td><td><strong>{item.trip.driver}</strong><small>Viaje N.º {item.trip.id}</small></td><td><strong>{item.trip.vehicle}</strong></td><td className="operational-consumption"><strong>{item.consumption ? item.consumption.toFixed(2).replace(".", ",") : "Sin ciclo"}</strong></td><td>{item.capacity}</td><td>{item.matchingOrders.length}</td><td>{item.km ? `${number.format(Math.round(item.km))} km` : "—"}</td><td>{money.format(item.invoiced)}</td></tr>)}{!operationalRows.length && <tr><td colSpan={9} className="no-results">No hay viajes para los filtros seleccionados.</td></tr>}</tbody></table></div></section>}
...
function FleetModule({ vehicles, setVehicles, maintenance, setMaintenance, trips, fuelEntries, branches, onToast }: {
  ...
  return <section className="fleet-layout">
    ...
    <div className="table-card fleet-list-card"><div className="card-heading"><div><p className="eyebrow">Vehículos registrados</p><h2>Lista de la flota</h2></div><strong>{vehicles.length} vehículos</strong></div><div className="table-scroll"><table className="fleet-list"><thead><tr><th>Chapa / vehículo</th><th>Sucursal</th><th>Kilometraje</th><th>Viajes</th><th>Meta combustible</th><th>Próximo mantenimiento</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{vehicles.map((vehicle) => {
      ...
      const fuelTarget = vehicle.fuelConsumptionTarget ?? defaultFuelTargets[vehicle.plate] ?? 0;
      return <tr key={vehicle.id} className={fleetStatus !== "Activo" ? "inactive-row" : ""}>
        <td><div className="fleet-vehicle-cell"><span className="fleet-truck"><Icon name="vehicle"/></span><div><strong>{vehicle.plate}</strong><small>{[vehicle.brand, vehicle.model, vehicle.year].filter(Boolean).join(" · ") || vehicle.type || "Ficha técnica pendiente"}</small>{vehicle.odometerStatus === "Averiado" && <span className="odometer-badge">⚠ Odómetro averiado</span>}{last && <small>Último: {last.description}</small>}</div></div></td>
...
function TripTable({ trips, rates, onAll, onEdit, onFinish, onReport, onDelete }: { trips: Trip[]; rates: FreightRates; onAll?: () => void; onEdit: (trip: Trip) => void; onFinish: (trip: Trip) => void; onReport: (trip: Trip) => void; onDelete?: (trip: Trip) => void }) {
  trips = [...trips].sort((a, b) => b.startDate.localeCompare(a.startDate) || b.id - a.id);
